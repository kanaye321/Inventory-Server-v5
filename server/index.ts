...
async function ensureSslCertificates(): Promise<void> {
  ...
  // If env vars already point to existing files, nothing to do
  if (
  ...
  // Auto-generate certificate when files don't exist yet
  if (!fs.existsSync(certFile) || !fs.existsSync(keyFile)) {
    console.log("🔐 Generating self-signed SSL certificate...");
    try {
      const { default: selfsigned } = await import("selfsigned");

      // Collect local IP addresses for the certificate
      ...
      console.log(`   Valid IPs: ${ipAddresses.join(", ")}`);
      console.log(``);
      console.log(`⚠️  IMPORTANT: To remove the "Not Secure" browser warning, run this`);
      console.log(`   PowerShell script ONCE as Administrator on your Windows server:`);
      console.log(``);
      console.log(`   Right-click PowerShell → "Run as Administrator" → paste:`);
      console.log(`   Import-Certificate -FilePath "${certFile}" -CertStoreLocation Cert:\\LocalMachine\\Root`);
      console.log(``);
      console.log(`   Or run the included script: .\\trust-ssl-cert.ps1`);
      console.log(`   After running it, restart your browser.`);
    } catch (err) {
      console.warn("⚠️  Could not generate SSL certificate:", (err as Error).message);
      console.warn("    Falling back to HTTP mode.");
      return;
    }
  }

  // Point env vars to the generated files
  process.env.SSL_CERT_FILE = certFile;
  process.env.SSL_KEY_FILE  = keyFile;
}
...
// Register Page Builder routes
  ...
  server.listen({
    ...
    if (isHttps) {
      console.log(`🔒 HTTPS enabled - accessible securely from other devices`);
    }
    console.log();
